...
const OPENROUTER_IMAGE_GENERATION_TIMEOUT_MS = 240_000
const OPENROUTER_GENERATION_IMAGE_MAX_WIDTH = 1024
const OPENROUTER_GENERATION_IMAGE_MAX_HEIGHT = 2048
const OPENROUTER_GENERATION_IMAGE_QUALITY = 0.85
...
export async function generateImage({
  ...
  try {
    ...
    const resizedReferenceImage = await resizeImageForOpenRouterGeneration(beforeImageUrl)
        ...
        messages: [
          {
            ...
            content: [
              {
                type: 'text',
                text: buildImagePrompt(originalAnalysis, designSystem, spec),
              },
              {
                type: 'image_url',
                image_url: { url: resizedReferenceImage.dataUrl },
              },
            ],
          },
        ],
...
function normalizeImageGenerationError(error: unknown) {
  if (isAbortError(error)) {
    return new Error(
      'OpenRouter GPT Image 2 image generation timed out after 240 seconds.',
    )
  }
...
function getAfterImageStatus(error: Error) {
  if (
    ...
    return 'OpenRouter GPT Image 2 image generation timed out after 240 seconds.'
  }
...
async function resizeImageForOpenRouterGeneration(beforeImageUrl: string) {
...
function buildImagePrompt(
  ...
  return [
    'Generate and return one final improved detail page image.',
    'Do not return only text.',
    'Use the uploaded image as visual reference.',
    'Preserve the original product/service information as much as possible.',
    'Improve layout, readability, hierarchy, and conversion structure.',
    'Avoid unsupported medical/legal/effectiveness claims.',
    '',
    'Create an improved beauty ecommerce detail-page style output image based on the uploaded reference image and improved_design_spec.',
